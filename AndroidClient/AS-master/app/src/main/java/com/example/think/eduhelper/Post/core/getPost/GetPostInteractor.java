...
public class GetPostInteractor implements GetPostConstractor.Interactor {
    ...
    @Override
    public void getAcceptedPostsFromFirebase() {
        final String curr_UID = FirebaseAuth.getInstance().getCurrentUser().getUid();
        FirebaseDatabase.getInstance().getReference().child(Constants.ARG_SELECTED_POSTS).child(curr_UID).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                Iterator<DataSnapshot> dataSnapshots = dataSnapshot.getChildren().iterator();
                List<Post> posts = new ArrayList<>();
